 ...
 * screen already read one. That would have made the **portal import a dex
 * service**, and the standing rule in this repo is that it may not: `/dex`
 * and `/website` are two products sharing one chassis and nothing else. The
 * rule exists so a dex refactor cannot break the company site and portal
 * copy cannot leak into a dex screen.
 *
 * The address is genuinely neither product's. It belongs to the studio, and
 * the portal's CONTACT US page and the dex's SUPPORT screen are two places
 ...
 * encyclopedia, about the portal's navigation, or about either one's state.
 */
