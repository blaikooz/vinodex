...
describe('dataset coverage', () => {
  ...
  /** The per-category counts iOS pins, to the entry. */
  it('matches the iOS per-category counts', () => {
    expect(countIn('GRAPES')).toBe(80);
    expect(countIn('REGIONS')).toBe(60);
    expect(countIn('STYLES')).toBe(29);
    expect(countIn('CONTINENTS')).toBe(6);
    expect(countIn('FLAVORS')).toBe(109);
  });

  /**
   * The five categories iOS counts must still total its 284 — the number the
   * DATA panel shows on both platforms.
   */
  it('totals the same 284 entries iOS reports', () => {
    const shared =
      countIn('GRAPES') + countIn('REGIONS') + countIn('STYLES') + countIn('FLAVORS') + countIn('CONTINENTS');
    expect(shared).toBe(284);
  });
  ...
  it('draws regions from the eighteen countries iOS counts', () => {
    const origins = new Set(
      all.filter(isRegionEntry).map(e => e.details.origin).filter((o): o is string => !!o),
    );
    expect(origins.size).toBe(18);
  });
