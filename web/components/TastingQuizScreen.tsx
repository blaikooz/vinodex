    ...
    return (
      <DeviceLayout title={mode === 'daily' ? 'DAILY CHALLENGE' : 'WINE EXAM'} showBack onBack={onBack} onHome={onHome} centerHeaderText>
        <div className="h-full flex flex-col items-center justify-center gap-4 p-6 text-center" style={{ backgroundColor: 'var(--lcd-page)' }}>
          {passed ? <CheckCircle2 size={72} className="text-green-400" /> : <XCircle size={72} className="text-red-400" />}
          ...
          <div className="flex gap-3 mt-2">
            {mode === 'practice' && (
              <button onClick={() => { setSession(retrySession(session)); setNewlyUnlocked(null); }} className="rounded-xl bg-stone-800 border-b-4 border-stone-950 active:translate-y-0.5 px-5 py-3 font-retro text-[0.6rem] tracking-widest text-green-300">RETRY</button>
            )}
            <button
              onClick={() => (mode === 'daily' ? onBack() : (setSession(null), setNewlyUnlocked(null)))}
              className="rounded-xl bg-green-600 border-b-4 border-green-800 active:translate-y-0.5 px-5 py-3 font-retro text-[0.6rem] tracking-widest text-white"
            >
      ...
      <DeviceLayout title={mode === 'daily' ? 'DAILY CHALLENGE' : 'WINE EXAM'} showBack onBack={onBack} onHome={onHome} centerHeaderText>
        <div className="relative h-full">
          <div className="h-full overflow-y-auto custom-scrollbar p-4 flex flex-col gap-4" style={{ backgroundColor: 'var(--lcd-page)' }}>
            ...
            <p className="font-mono text-base text-stone-100 normal-case leading-snug">{question.prompt}</p>
          ...
          {answered && answer && (
            <div className="absolute inset-0 z-40 flex items-end justify-center p-4">
              ...
              <div className="relative w-full max-w-sm rounded-2xl border-2 border-green-700 bg-stone-900 p-4 flex flex-col gap-3 mb-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => onOpen(answer)} className="flex items-center gap-1 rounded-xl bg-stone-800 border-b-4 border-stone-950 active:translate-y-0.5 px-3 py-2.5 font-retro text-[0.55rem] tracking-widest text-green-300">
                    <BookOpen size={13} /> LEARN MORE
                  </button>
                  <button onClick={next} className="flex-1 rounded-xl bg-green-600 border-b-4 border-green-800 active:translate-y-0.5 px-3 py-2.5 font-retro text-[0.55rem] tracking-widest text-white">
  ...
  return (
    <DeviceLayout title="WINE EXAM" showBack onBack={onBack} onHome={onHome} centerHeaderText>
      <div className="h-full overflow-y-auto custom-scrollbar p-4 flex flex-col gap-3" style={{ backgroundColor: 'var(--lcd-page)' }}>
        ...
        <p className="font-mono text-xs text-stone-500 text-center mt-2 normal-case">Ten questions, 8 to pass. Passing a paper unlocks the next one.</p>
      </div>
      ...
      {lockedTier && (
        <div className="absolute inset-0 z-40 bg-black/80 flex items-center justify-center p-6">
          <div className="w-full max-w-xs bg-stone-900 border-2 border-yellow-700 rounded-lg p-5 flex flex-col gap-3 text-center">
            <Lock size={28} className="text-yellow-400 self-center" />
