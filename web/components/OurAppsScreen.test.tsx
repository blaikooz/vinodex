...
describe('<OurAppsScreen />', () => {
  ...
  it('lists Vinodex and an honest placeholder for the rest', () => {
    renderShelf();
    // By role, not by text (0.6.5 merge): the parity line's chassis renders a
    // VINODEX wordmark of its own, so a bare `getByText('VINODEX')` now matches
    // two nodes and throws. The app tile is the one that is a button.
    expect(screen.getByRole('button', { name: /VINODEX/ })).toBeDefined();
